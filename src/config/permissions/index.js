...
import { PERMISSIONS, request, RESULTS, openSettings, check, checkMultiple, requestMultiple } from 'react-native-permissions';
...
const checkPhotoGalleryPermission = async (cb) => {
    ...
    switch (result) {
        ...
        case RESULTS.BLOCKED:
            showAlertBox(Platform.OS == "android" ? "media storage" : 'photo gallery');
            break;
...
const multiplePermissionLocation = Platform.OS == 'android'
...
const checkLocationPermission = async (cb) => {
    ...
    if (Platform.OS == 'android') {
        if (result["android.permission.ACCESS_FINE_LOCATION"] == RESULTS.GRANTED 
        ||
        ...
        } else if (result["android.permission.ACCESS_FINE_LOCATION"] == RESULTS.DENIED || result["android.permission.ACCESS_COARSE_LOCATION"] == RESULTS.DENIED) {
            requestLocationPermission();

        } else if (result["android.permission.ACCESS_FINE_LOCATION"] == RESULTS.BLOCKED || result["android.permission.ACCESS_COARSE_LOCATION"] == RESULTS.BLOCKED) {
            showAlertBox('location');

        } else if (result["android.permission.ACCESS_FINE_LOCATION"] == RESULTS.UNAVAILABLE || result["android.permission.ACCESS_COARSE_LOCATION"] == RESULTS.UNAVAILABLE) {
            showAlertBox('N/A');
        }
    }
    ...
    // For iOS Permissions
        ...
        if (result["ios.permission.LOCATION_ALWAYS"] == RESULTS.GRANTED ||
        ...
        } else if (result["ios.permission.LOCATION_ALWAYS"] == RESULTS.DENIED || result["ios.permission.LOCATION_WHEN_IN_USE"] == RESULTS.DENIED) {
            requestLocationPermission(cb);

        } else if (result["ios.permission.LOCATION_ALWAYS"] == RESULTS.BLOCKED || result["ios.permission.LOCATION_WHEN_IN_USE"] == RESULTS.BLOCKED) {
            showAlertBox('location');

        } else if (result["ios.permission.LOCATION_ALWAYS"] == RESULTS.UNAVAILABLE || result["ios.permission.LOCATION_WHEN_IN_USE"] == RESULTS.UNAVAILABLE) {
            showAlertBox('N/A');
        }
    }
}
...
const requestLocationPermission = async (cb) => {
    ...
    // For Android Permissions
    if (Platform.OS == 'android') {
        if (result['android.permission.ACCESS_FINE_LOCATION'] == RESULTS.GRANTED || result['android.permission.ACCESS_COARSE_LOCATION'] == RESULTS.GRANTED) {
            cb();
        }

        if (result['android.permission.ACCESS_FINE_LOCATION'] == RESULTS.BLOCKED || result['android.permission.ACCESS_COARSE_LOCATION'] == RESULTS.BLOCKED) {
            showAlertBox('location');
        }

    } else if (Platform.OS == 'ios') {
        if (result['ios.permission.LOCATION_ALWAYS'] == RESULTS.GRANTED || result['ios.permission.LOCATION_WHEN_IN_USE'] == RESULTS.GRANTED) {
            cb();
        }
        else if (result['ios.permission.LOCATION_ALWAYS'] == RESULTS.BLOCKED || result['ios.permission.LOCATION_WHEN_IN_USE'] == RESULTS.BLOCKED) {
            showAlertBox('location');
